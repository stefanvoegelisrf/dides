<template>
    <main class="overflow-hidden">
        <img src="/images/meet-me/vlcsnap-2024-05-01-14h44m07s901.png" alt="San Junipero" class="h-screen object-cover">
        <p class="absolute top-40 left-1/2 -translate-x-1/2 nanum-myeongjo text-xl xl:text-4xl w-max">“Hello, stupid. It’s
            good to see you.”</p>
        <h1 class="absolute left-1/2 -translate-x-1/2  bottom-0 text-7xl xl:text-8xl m-plus-1p-medium w-max">SAN JUNIPERO</h1>
        <h2 class="absolute bottom-20 xl:bottom-28 left-9 xl:left-1/3 m-plus-1p-regular">Black Mirror Series 3, Episode 4</h2>
        <h2 class="absolute bottom-20 xl:bottom-28 right-9 xl:right-1/3 m-plus-1p-regular">Directed by Owen Harris</h2>
    </main>
</template>
<style scoped>
@font-face {
    font-family: 'Nanum Myeongjo';
    font-style: normal;
    font-weight: 400;
    font-display: swap;
    src: url('/fonts/Nanum_Myeongjo/NanumMyeongjo-Regular.ttf') format('truetype');
    unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+0304, U+0308, U+0329, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD;
}

.nanum-myeongjo {
    font-family: 'Nanum Myeongjo', serif;
}

@font-face {
    font-family: 'M PLUS 1p Regular';
    font-style: normal;
    font-weight: 400;
    font-display: swap;
    src: url('/fonts/M_PLUS_1p/MPLUS1p-Regular.ttf') format('truetype');
    unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+0304, U+0308, U+0329, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD;
}

@font-face {
    font-family: 'M PLUS 1p Medium';
    font-style: normal;
    font-weight: 400;
    font-display: swap;
    src: url('/fonts/M_PLUS_1p/MPLUS1p-Medium.ttf') format('truetype');
    unicode-range: U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+0304, U+0308, U+0329, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD;
}

.m-plus-1p-regular {
    font-family: "M PLUS 1p Regular", sans-serif;
}

.m-plus-1p-medium {
    font-family: "M PLUS 1p Medium", sans-serif;
}
</style>